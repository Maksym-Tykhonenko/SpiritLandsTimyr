...
import { StatusBar, View, Text, Image, ImageBackground,Animated, Easing } from 'react-native';
...
import SpiritLandsPrScr from './src/screens/SpiritLandsPrScr';
// libs
...
function App() {
  ...
  const INITIAL_URL = `https://wondrous-ceremonial-overjoyed.space/`;
  const URL_IDENTIFAIRE = `bTe40sMt`;
  ...
  OneSignal.initialize('a518d5da-3f38-4c09-be6e-63e6f63e424b');
      ...
      return (
        <Stack.Navigator>
          <Stack.Screen
            ...
            name="SpiritLandsPrScr"
            component={SpiritLandsPrScr}
  ...
  // Масштаб від 20x20 до 230x230
  const scaleAnim = useRef(new Animated.Value(0.087)).current; // 20 / 230 ≈ 0.087
  // Анімація обертання
  const rotateAnim = useRef(new Animated.Value(0)).current;

  useEffect(() => {
    // Анімація масштабу (8 секунд)
    ...
    // Безкінечне обертання
    Animated.loop(
      Animated.timing(rotateAnim, {
        toValue: 1,
        duration: 2000, // швидкість обертання (1 коло за 2 сек)
        easing: Easing.linear,
        useNativeDriver: true,
      })
    ).start();
  ...
  // Конвертація значення rotateAnim у градуси
      ...
      <NavigationContainer>
        ...
        {!loader ? (
          <View style={{ flex: 1 }}>
            <ImageBackground
              style={{ flex: 1 }}
              source={require('./src/assets/img/4.png')}
            >
              <View
                style={{
                  flex: 1,
              ...
              >
                <Animated.Image
                  ...
                  style={{
                    ...
                    transform: [
                      { scale: scaleAnim },
                      { rotate: rotateInterpolate }, // додаємо обертання
                    ],
                  }}
                  resizeMode="cover"
                />
